...
state = {
  client_id: 'Y414UGHZIJ5SOPN5NT2WATWMYTTNJRNOIY2IJ0R0CREKLJX0',
  client_secret: 'M1TVJLFZC1HKUNAUXOF1FW3W1IIP1T0RTEPEYGTSKV50OGNM',
  searches: [],
  items: [],
  geojson: {
...
// add markers to geojson object
  function mapLatLong(lat,lng, title, url, query, address){
    return {
      type: 'Feature',
      geometry: {
        type: 'Point',
...
//write previous results
  function writeResults(state){
    for (var i=0; i<state.searches.length; i++) {
          ...
          var html= '';
          html = '<a><h3 class="searchLink">'+ state.searches[i].search +'</h2></a>';
...
var GetData = function(state, query){
  ...
  $.ajax({
  dataType: "json",
  url: 'https://api.foursquare.com/v2/venues/search?ll='+globLat + ',' + globLong + '&query=' + $('#queryfood').val() + '&client_id=' + state.client_id + '&client_secret=' + state.client_secret + '&v=20170301',
    ...
    for (var i = 0;i < data.response.venues.length; i ++)
        {
          ...
          name1 = data.response.venues[i].name;
          if (address == null){
            address = "Whoops, looks like they don't have an address :("
          }
          if (url == null){
            var newName = name1.replace(" ", "+");
            url =  "<a href=http://www.google.com/#q=" + newName + " target='_blank'>" + "No website here, check google!" + "</a>" + "<br>" + address;
          }
          else {
            url = "<a href="+ url + " target='_blank'>" + url + "</a>" + "<br>" + address;
          }
          addItem(state,item);
          searchResults.geojson.features.push(mapLatLong(long,lat,title,url,query, address))
        } //end of loop
